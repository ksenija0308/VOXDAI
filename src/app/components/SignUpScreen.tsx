...
export default function SignUpScreen({ formData, updateFormData, nextScreen, onShowSignIn }: SignUpScreenProps) {
  ...
  const handleOAuthSignup = (provider: 'LinkedIn' | 'Google') => {
    if (!formData.acceptTerms) {
  ...
  const openOAuthModal = (provider: 'LinkedIn' | 'Google') => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="space-y-4">
          ...
          <Button
            ...
            onClick={() => openOAuthModal('LinkedIn')}
          >
      ...
      {/* OAuth Modal */}
      <Dialog open={showOAuthModal} onOpenChange={closeOAuthModal}>
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
            <div className="flex items-center justify-center mb-2">
              {selectedProvider === 'LinkedIn' ? (
                <svg className="w-12 h-12" fill="#0A66C2" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              ) : (
            ...
            <DialogTitle className="text-center" style={{ fontFamily: 'Helvetica, Arial, sans-serif', fontWeight: 'bold', fontSize: '24px' }}>
              Sign in to {selectedProvider}
            </DialogTitle>
            <DialogDescription className="text-center" style={{ fontFamily: 'Inter, sans-serif' }}>
              to continue to VOXD
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {/* Email Input */}
            <div>
              <label htmlFor="oauth-email" className="block mb-2" style={{ fontSize: '14px', fontWeight: '600' }}>
                Email
              </label>
              <Input
                id="oauth-email"
                type="email"
                placeholder={selectedProvider === 'LinkedIn' ? 'Email or phone' : 'Email'}
            ...
            {/* Password Input */}
            <div>
              <label htmlFor="oauth-password" className="block mb-2" style={{ fontSize: '14px', fontWeight: '600' }}>
                Password
              </label>
              <Input
                id="oauth-password"
            ...
            <div className="text-right">
              <a 
                href="#" 
                className={selectedProvider === 'LinkedIn' ? 'text-[#0A66C2]' : 'text-[#1a73e8]'}
            ...
            {/* OAuth Permissions */}
            <div className="bg-[#f3f3f5] p-4 rounded-lg space-y-2">
              <p style={{ fontSize: '13px', fontWeight: '600', fontFamily: 'Inter, sans-serif' }}>
                VOXD will receive:
              </p>
              <ul className="space-y-1.5 text-[#717182]" style={{ fontSize: '13px', fontFamily: 'Inter, sans-serif' }}>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5">•</span>
                  <span>Your name, profile photo, and email address</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="mt-0.5">•</span>
                  <span>Your professional experience and education</span>
                </li>
                {selectedProvider === 'LinkedIn' && (
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5">•</span>
                    <span>Your LinkedIn profile information</span>
                  </li>
                )}
              </ul>
            </div>

            {/* Terms and Conditions for OAuth */}
            <div className="flex items-start gap-3">
              <Checkbox
                id="oauth-terms"
                checked={formData.acceptTerms}
                ...
                className="mt-1"
              />
              <label htmlFor="oauth-terms" className="text-[#717182] cursor-pointer" style={{ fontSize: '14px' }}>
                I accept VOXD's{' '}
                <a href="#" className="text-[#0B3B2E] underline">Terms and Conditions</a>
                {' '}and{' '}
                <a href="#" className="text-[#0B3B2E] underline">Privacy Policy</a>
          ...
          <div className="flex gap-3">
            ...
            <Button
              className="flex-1 h-11"
              style={{ 
                backgroundColor: selectedProvider === 'LinkedIn' ? '#0A66C2' : '#1a73e8',
                color: 'white',
                fontFamily: 'Inter, sans-serif', 
                fontWeight: '600' 
              }}
              onClick={() => handleOAuthSignup(selectedProvider as 'LinkedIn' | 'Google')}
            >
              Sign in
            </Button>
          </div>
          ...
          {/* Provider Terms */}
          <p className="text-center text-[#717182] mt-2" style={{ fontSize: '12px', fontFamily: 'Inter, sans-serif' }}>
            By continuing, you also agree to {selectedProvider}'s{' '}
